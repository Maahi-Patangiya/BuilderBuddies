    ...
    <div style={{ fontFamily: 'system-ui', maxWidth: '1200px', margin: '0 auto', padding: '30px' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #f1f5f9', paddingBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <button onClick={() => setPortalMode('expert')} style={{ padding: '10px 18px', background: portalMode === 'expert' ? '#10b981' : '#f1f5f9', color: portalMode === 'expert' ? 'white' : '#334155', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', transition: 'all 0.2s' }}>NGO Backoffice Portal</button>
      ...
      {portalMode === 'worker' ? (
        <main style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', marginTop: '35px' }}>
          ...
          <section style={{ background: '#f8fafc', padding: '30px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
            ...
            <form onSubmit={submitWorkerQuery} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <button type="submit" style={{ padding: '14px', background: '#2563eb', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', fontSize: '15px' }}>Transmit Question to Queue</button>
            </form>
          </section>

          {/* Database FAQ Reference Column */}
          <section>
            <h2 style={{ marginTop: 0, color: '#1e293b' }}>📚 Direct Database Knowledge Base FAQ</h2>
        ...
        <main style={{ marginTop: '35px' }}>
          ...
          <div style={{ background: '#f0fdf4', padding: '25px', borderRadius: '16px', border: '1px solid #bbf7d0', marginBottom: '30px' }}>
            <h2 style={{ marginTop: 0, color: '#166534' }}>🛠️ Live NGO Helpdesk Triage Desk</h2>
            <p style={{ margin: 0, color: '#14532d', fontSize: '15px' }}>This feed updates **automatically** whenever a logged-in user enters a support query. Peer reviewers checking validation links can watch execution data streams land instantly without refreshing the browser frame.</p>
          </div>
          
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            <h3 style={{ color: '#1e293b', margin: '0 0 5px 0' }}>Active Incoming Infrastructure Streams ({submittedQuestions.length})</h3>
